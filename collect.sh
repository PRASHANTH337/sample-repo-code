...
TOTAL_TIER3=$(awk -F"," '{x+=$8}END{print x}' ./$TMP_FILE_NAME.csv)
...
echo "Total Instances: $TOTAL_INSTANCES,Total Processors: $TOTAL_PROCESSORS,Total Memory: $TOTAL_MEMORY,Total TIER1: $TOTAL_TIER1,Total TIER3: $TOTAL_TIER3,******,******,******" >> $TMP_FILE_NAME-sorted.csv
...
if [[ "$*" == *--historical* ]]; then
	echo "$TODAY,$TIME,$TOTAL_INSTANCES,$TOTAL_PROCESSORS,$TOTAL_MEMORY,$TOTAL_TIER1,$TOTAL_TIER2" >> $TMP_FILE_NAME-historical.csv
fi

if [[ "$*" == *--teams* ]]; then
	./allocation-by-team.sh ./allocation ./powervs_instances_*-sorted.csv
fi
